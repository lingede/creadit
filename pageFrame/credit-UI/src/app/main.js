...
var app = angular.module('app', ['ui.load', 'ui.router', 'ngStorage', 'brantwills.paging', 'oc.lazyLoad', 'ngImgCrop','highcharts-ng','angularFileUpload']);
    ...
    function($scope,   $localStorage,   $window ,$http,$state,$rootScope) {
        ...
        $scope.logout = function(){
            return $http({
                ...
                if(d.httpCode==200){//注销成功
                    deleteUserInfo();
                    window.location.href='http://idas.uestc.edu.cn/authserver/logout?service=http://www.ge.uestc.edu.cn';
		...
		$.ajaxSetup({
			...
			beforeSend: function(evt, request, settings) {
				//request.url = 'credit-Web' + request.url;
			},
