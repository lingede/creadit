...
angular.module('app')
	.controller('actroundController', ['$rootScope', '$scope', '$http', '$state', '$timeout', 'toaster', 'FileUploader',
		function($rootScope, $scope, $http, $state, $timeout, toaster, FileUploader) {
			...
			$scope.selectAll = function($event) {
				...
				for(var i = 0; i < $scope.pageInfo.list.length; i++) {
					...
					if($scope.pageInfo.list[i].reviewStatus == 0 && $scope.pageInfo.list[i].isaffirm == 1) {
				...
				layui.use(['layer'], function() {
					...
					layer.open({
						...
						btn2: function(index, layero) {
							$state.go('main.platform.actround.list');
							layer.close(index);
						},
						cancel: function() {
							$state.go('main.platform.actround.list');
