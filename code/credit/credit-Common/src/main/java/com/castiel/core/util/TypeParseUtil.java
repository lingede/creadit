...
import com.castiel.core.exception.DataParseException;
import com.castiel.core.support.DataType;
		...
		try {
			if ("String".equalsIgnoreCase(type) || DataType.STRING.equalsIgnoreCase(type)) {
				...
				try {
					...
					try {
						fieldDate = new Date(Long.parseLong(str));
					} catch (Exception e) {
						throw new DataParseException("in com.castiel.core.util.TypeParseUtil 360 line,Parse Date Error,Maybe DataType of recordBean.update(create)Time is  java.Long.long",e);
					}
				}
